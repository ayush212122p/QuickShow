...
const bookTickets = async ()=>{
  try {
    ...
    if(!selectedTime || !selectedSeats.length) return toast.error("Please select time and seats first");
...
<div className='mt-5 space-y-1'>
  {show.dateTime[date].map((item)=>( 
    <div key={item} onClick={()=>setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ? "bg-primary text-white" : "hover:bg-primary/20"}`}>
