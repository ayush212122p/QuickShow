...
// Inngest fn to cancel booking and release seats of show after 10 minutes of booking id payment is not done
...
// Inngest fn to send email when user book a show
...
const sendShowReminders = inngest.createFunction(
    ...
    async ({step}) =>{
        const now = new Date();
        const in8Hours = new Date(now.getTime() + 80 * 60 * 60 * 1000);
        ...
        if(reminderTasks.length ===0){
            return {sent:0,message:"No-reminsers to send."}
        }
...
// Inngets fn to send notifications when a new show is added
